export const translations = {
  ru: {
    meta: {
      title: 'Hovsep Hunanyan — Портфолио',
      description: 'Персональное портфолио разработчика',
    },
    nav: {
      hero: 'Главная',
      about: 'Обо мне',
      skills: 'Навыки',
      projects: 'Проекты',
      contact: 'Контакты',
    },
    hero: {
      greeting: 'Привет, я',
      viewProjects: 'Смотреть проекты',
      contact: 'Связаться',
    },
    sections: {
      about: 'Обо мне',
      skills: 'Навыки',
      projects: 'Проекты',
      contact: 'Контакты',
    },
    profile: {
      title: 'Frontend-разработчик',
      tagline: 'Создаю современные и удобные веб-приложения',
      about: [
        'Привет! Я разработчик с фокусом на создание красивых и функциональных интерфейсов.',
        'Люблю чистый код, продуманный UX и постоянно изучаю новые технологии.',
      ],
      location: 'Армения, Абовян',
    },
    projects: {
      films: {
        description: 'Сайт о фильмах с каталогом, описаниями и удобной навигацией.',
      },
      forthub: {
        description: 'Проект, посвящённый Fortnite — персонажи, предметы и игровая информация.',
      },
      taskflow: {
        description: 'Приложение для управления задачами с проектами и вложенными todo-списками.',
      },
    },
    contact: {
      text: 'Есть интересный проект или просто хотите поздороваться? Напишите — отвечу!',
      button: 'Написать мне',
    },
    footer: {
      madeWith: 'Сделано с ❤️',
    },
    common: {
      demo: 'Демо',
      openProject: 'Открыть проект',
      menu: 'Меню',
      language: 'Язык',
    },
  },
  en: {
    meta: {
      title: 'Hovsep Hunanyan — Portfolio',
      description: 'Personal developer portfolio',
    },
    nav: {
      hero: 'Home',
      about: 'About',
      skills: 'Skills',
      projects: 'Projects',
      contact: 'Contact',
    },
    hero: {
      greeting: "Hi, I'm",
      viewProjects: 'View projects',
      contact: 'Get in touch',
    },
    sections: {
      about: 'About me',
      skills: 'Skills',
      projects: 'Projects',
      contact: 'Contact',
    },
    profile: {
      title: 'Frontend Developer',
      tagline: 'Building modern, user-friendly web applications',
      about: [
        "Hi! I'm a developer focused on creating beautiful and functional interfaces.",
        'I love clean code, thoughtful UX, and constantly learning new technologies.',
      ],
      location: 'Abovyan, Armenia',
    },
    projects: {
      films: {
        description: 'A film website with a catalog, descriptions, and easy navigation.',
      },
      forthub: {
        description: 'A Fortnite project featuring characters, items, and game information.',
      },
      taskflow: {
        description: 'A task management app with projects and nested todo lists.',
      },
    },
    contact: {
      text: 'Have an interesting project or just want to say hi? Drop me a line — I will reply!',
      button: 'Say hello',
    },
    footer: {
      madeWith: 'Made with ❤️',
    },
    common: {
      demo: 'Demo',
      openProject: 'Open project',
      menu: 'Menu',
      language: 'Language',
    },
  },
}
